...
def parse_github_date(date_str):
    """Преобразует строку даты GitHub в datetime объект"""
    if not date_str:
        return None
    ...
    while True:
        ...
        for item in data:
            ...
            issues.append({
                ...
                "created_at": parse_github_date(item.get("created_at")),
                "closed_at": parse_github_date(item.get("closed_at")),
    ...
    while True:
        ...
        for item in data:
            ...
            commits.append({
                ...
                "commit_date": parse_github_date(author.get("date")),
    ...
    while True:
        ...
        for item in data:
            ...
            prs.append({
                ...
                "state": item.get("state", ""),
                "created_at": parse_github_date(item.get("created_at")),
                "merged_at": parse_github_date(item.get("merged_at")),
                "closed_at": parse_github_date(item.get("closed_at")),
                "author": item.get("user", {}).get("login", ""),
